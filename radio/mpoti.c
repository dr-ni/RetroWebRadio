...
#define DT_M 5//GPIO5 (P18) yellow
#define CLK_M 4//GPIO4 (P16) green
#define SW_M 6 //GPIO6 (P22) orange
...
void interrupt_MRot(void) {
        if(digitalRead(DT_M) && digitalRead(CLK_M))  {
            state_3 = STATE_NONE;
        }
...
void interrupt_MPush(void){
   if(!digitalRead(SW_M))
      usleep(5000); //debouncing
   if(!digitalRead(SW_M)){
      system ("/home/radio/radio/mpush");
      fprintf(stderr, "Middle button pushed\n");
   }
}
...
//    XInitThreads();  // to fix error xcb unknown request in queue while dequeuing 
    ...
    pinMode(CLK_M, INPUT);
    pinMode(SW_M,  INPUT);
    pullUpDnControl(SW_M, PUD_UP);
    wiringPiISR (DT_M, INT_EDGE_BOTH, interrupt_MRot) ;
    wiringPiISR (CLK_M, INT_EDGE_BOTH, interrupt_MRot) ;
    wiringPiISR (SW_M, INT_EDGE_FALLING, interrupt_MPush) ;
